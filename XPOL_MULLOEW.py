...
import shutil
import make_input
import replace_by_Q_MUL
import replace_by_Q_MUL_EOM
import charge_EOM
import gen_water_pc
import gen_main_pc
import charge_water
...
def XPOL_MULLOEW (L):
 ...
 replace_by_Q_MUL.replace_by_Q(main+'.xyz',main+'_pc.out',1,charge_type)#update uracil charges from 1st iter
 ...
 while (abs(Eo-En) >= epsilon):
   ...
   Eo = En
   replace_by_Q_MUL.replace_by_Q(main+'.xyz',main+'_pc.out',1,charge_type)#update uracil charges
   for i in range(n):
     replace_by_Q_MUL.replace_by_Q('Water'+str(i+1)+'.xyz','Water'+str(i+1)+'_pc.out',1,charge_type)#update water charges
   for i in range (n):
     pc_water.pc_water(i+1,n)
...
#Regenerating water charges from EE/IP/EA calc. of main frag.
 replace_by_Q_MUL_EOM.replace_by_Q_EOM(main+'.xyz',main+'_'+job+'_EOM_pc.out',1,charge_type)
 for i in range (n):
   gen_water_pc.gen_water_pc(i+1,main,N)#gen. pc files for water using updated uracil charges
