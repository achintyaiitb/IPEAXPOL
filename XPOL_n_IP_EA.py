...
def XPOL_n (L,iter_no):
 ...
 for k in range(iter_no):
  ...
  s = 'Time taken for iteration number '+str(k+1)+' for main fragment :\n'
  s += time+'\n\n'
  ...
  for i in range (n):
    ...
    time = comp_time.comp_time('Water'+str(i+1)+'_pc.out')#comp. time calc.
    s = 'Time taken for iteration number '+str(k+1)+' for water mol. no. '+str(i+1)+' :\n'
    s += time+'\n\n'
    f = open (main+'_time.txt','a')
    f.write(s)
    f.close()
